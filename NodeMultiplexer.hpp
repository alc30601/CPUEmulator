...
public:
    ...
    NodeMultiplexer(void)
    {
        ...
        gb.outto(Port(enty, 2), Ports{ Port(and1, 1) });
        gb.outto(Port(enty, 3), Ports{ Port(and2, 1) });
...
public:
    ...
    NodeDeMultiplexer(void)
    {
        ...
        gb.outto(Port(enty, 1), Ports{ Port(not1, 1), Port(and2, 2) });
        gb.outto(Port(enty, 2), Ports{ Port(and1, 1), Port(and2, 1) });
        gb.outto(Port(and1, 1), Ports{ Port(exit, 1) });
        gb.outto(Port(and2, 1), Ports{ Port(exit, 2) });
